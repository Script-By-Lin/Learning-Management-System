...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await communicationController.listDiscussions(courseId);
    if (!result.success) {
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await communicationController.createDiscussionPost(courseId, body, auth.user.userId);
    if (!result.success) {
      return NextResponse.json(result, { status: 400 });
    }
